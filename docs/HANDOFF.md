# Product Handoff: VEO Cinematic Sequence Production Tool

## What This Is

A multi-shot-first cinematic video production tool built on top of Google VEO. It generates structured multi-shot video sequences — not isolated single-shot clips.

The core workflow: a user provides a story or scene description, the system plans a sequence of cuts, each cut is decomposed into retention-optimized shots with distinct roles, and the full batch is submitted to VEO for generation.
...
**Structural honesty.** What the user sees in the editor is what gets sent to VEO. The export/preview shows the actual timestamp-based prompt payload, not a flattened prompt string.
...
| 용어 | 의미 | 레이어 |
|------|------|--------|
| **컷 (Cut/장면)** | 하나의 서사 단위. 8초 VEO 생성 단위 | Layer 1-2 |
...
Server Enforcement (Studio: block invalid | Batch: auto-repair)
    ↓
VEO API (timestamp prompts via Gemini API)
    ↓
...
**Server owns**: final model selection, prompt serialization, multi-shot policy enforcement, auto-repair (Batch), blocking (Studio), VEO API submission.
...
### Model Capabilities (VEO)

| Model | Max Shots | Min Shot Duration | Multi-Shot |
|-------|-----------|-------------------|------------|
| veo-text-to-video | 4 | 2s | Yes (2+2+2+2) |
| veo-image-to-video | 4 | 2s | Yes (2+2+2+2) |
...
2. **Server auto-repaired shots lack roles.** The VEO API's timestamp prompt format only supports `{timestamp, prompt}`. Server-generated repair shots use the base prompt for all shots without role semantics. The client retains role-enriched state in the editor.

3. **Video-edit workflow scaffolded but not wired.** Video editing is not yet supported in VEO.
...
1. **Server-side role propagation.** If VEO ever supports roles in their API, propagate client-side roles through to the provider payload.

2. **Video-edit workflow UI.** Wire a video editing interface when VEO supports it.
...
5. **Model fallback testing.** The capability registry includes fallback models, but the fallback path needs production validation.

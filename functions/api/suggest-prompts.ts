...
function buildHistoryMarketingPrompt(): string {
  return `너는 "역사 속 마케팅 전략 발굴 전문가"야. 과거 병원·의사·의료인들이 환자를 모으기 위해 **직접 실행한 마케팅·홍보·광고·브랜딩 행위**만 찾는다.
웹 검색으로 실존하는 역사적 사례를 찾아라. 매번 새로운 사례를 발굴해야 한다.

## ★★★ 가장 중요한 원칙: "마케팅 행위" vs "의학 발전" 엄격 구분 ★★★

이 콘텐츠의 핵심 질문: "그때도 마케팅을 했을 텐데, 어떻게 했을까?"
→ 답은 반드시 "구체적 마케팅 행위"여야 한다. 의학적 업적·발전·혁신·사회적 공헌은 절대 답이 될 수 없다.

### 자문 테스트 (3단계 — 하나라도 "아니오"면 즉시 제외):
1단계: "이 인물이 유명한 이유가 마케팅 전략인가, 아니면 의학적/사회적 업적인가?"
  → 업적이 먼저 떠오르면 제외.
2단계: "환자를 모으려는 상업적 의도로 실행한 구체적 홍보 행동이 있는가?"
  → 광고·전단지·간판·공개시연·무료진료(유치목적)·슬로건 등이 없으면 제외.
3단계: "의학 발전/사회 공헌/차별 극복 스토리를 빼면 마케팅 이야기만으로 콘텐츠가 성립하는가?"
  → 마케팅만으로 안 되면 제외.

### 구체적 제외 인물/패턴 (절대 사용 금지):
- ❌ "최초의 여성 의사", "최초의 흑인 의사", "최초의 원주민 의사" → 의학 발전/사회적 업적
  예: Elizabeth Blackwell, Elizabeth Garrett Anderson, Daniel Hale Williams, Susan La Flesche Picotte, James McCune Smith, Rebecca Lee Crumpler
- ❌ 의학 업적을 현대 마케팅 용어로 재해석: "허준의 동의보감 브랜딩", "화타의 마취술 혁신"
- ❌ 면허 투쟁·차별 극복·사회 운동 서사를 "차별화 전략"으로 포장
- ❌ "결과적으로 유명해졌다" = 마케팅이 아님

### ✅ 좋은 사례 (이런 걸 찾아라):
- 페인리스 파커: 법적 개명 + 거리 공개발치쇼 + 마차 퍼레이드 → 순수 광고/홍보 행위
- 켈로그: 요양원을 럭셔리 리조트로 리포지셔닝 + 부유층 타겟 광고
- 메이요 클리닉: "환자가 먼저" 슬로건 + 의사 급여제 차별화 브랜딩
- 19세기 치과 신문 광고: "무통 발치" 공포 해소 카피라이팅
- 에도시대 의사 무료 시술 순회: 입소문 마케팅
- 오스만 하맘 건강 상담 부스: 유동인구 밀집지 환자 유치

## ★ 주인공 필수 규칙 ★
- 모든 카드에는 반드시 특정 실존 인물 1명이 주인공으로 있어야 함
- 기관/병원 이름만으로 진행 금지. "메이요 클리닉" → "윌리엄 메이요", "배틀크릭 요양원" → "존 하비 켈로그"
- title에 주인공 이름이 반드시 포함되어야 함
...
## 필수 포함 조건 — 아래 중 1개 이상의 구체적 홍보 행동이 반드시 있어야 함
- 신문/잡지 광고, 전단지/포스터/간판 제작
- 무료 진료/강연/시연을 환자 유치 목적으로 기획
- 병원 이름/슬로건/브랜드를 의도적으로 설계 (개명, 포지셔닝)
- 특정 환자층 타겟팅 메시지, 언론 의도적 노출, 환자 경험 설계
- 입지 전략 (유동인구 밀집지 선택), 가격 전략, 패키지 상품화

## 제외 (위반 시 카드 무효 — 하나라도 해당되면 절대 사용 금지)
- ❌ 의학 업적·수술 성공·기술 혁신·의학 발전 (마케팅 아님)
- ❌ "최초의 ~" 인물 (최초의 여성의사, 최초의 흑인의사 등 → 업적 서사)
- ❌ 면허 투쟁/차별 극복/사회 운동을 "마케팅 전략"으로 재해석
- ❌ 약국/약사/약방 (병원/의사만)
- ❌ 특허약(patent medicine) 판매자·발명가·사업가 (의사 면허 없는 자 전부 제외)
- ❌ 학술 저술·의학서 집필 (환자 유치 의도 명확한 대중 건강서만 예외)
- ❌ "결과적으로 유명해졌다" = 마케팅 아님

## 절대 원칙
1. 실존 인물/기관만. 2. 의사 면허를 가진 의사·병원만. 3. 카드의 핵심은 "마케팅 행위 자체"여야 한다 (의학 발전·사회 공헌 절대 금지). 4. 사실 검증 가능.

## 출력 필드 규칙
title (15~30자): "[실존 인물 이름] + [구체 마케팅 수단] + [결과]" (기관명만 쓰지 말고 반드시 인물명 포함)
hook (20~40자): MZ세대 말투 ("~임", "~임ㅋㅋ")
marketingTactic (10~20자): 핵심 환자 유치 수단 (마케팅 행위만, 업적/발전 금지)
region (5~15자): 지역/문화권

...
function validCombinations(targetSec: number): string {
  const combos: string[] = [];
  for (let dur = DURATION_MIN; dur <= DURATION_MAX; dur++) {
...
- 3~5초: 짧고 임팩트 있는 액션 컷, 빠른 템포
- 6~8초: 표준 드라마 컷, 감정 전환 포함
- 9~12초: 긴 호흡의 감정 씬, 대사가 있는 장면
- 13~15초: 긴 씬, 복잡한 행동

JSON으로만 응답 (recommendedCuts는 반드시 4~10 사이):
{
  "recommendedCuts": 숫자(4~10),
  "recommendedDuration": 숫자(${DURATION_MIN}~${DURATION_MAX} 사이 정수),
  ...
  "scenes": ["장면1 요약", "장면2 요약", ...]
    ...
    const rawDur  = parsed.recommendedDuration ?? DURATION_FALLBACK;
